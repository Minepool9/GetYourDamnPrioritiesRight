...
//        while (t != null)
//        {
//            if (t.TryGetComponent<EnemyIdentifier>(out var e) && e.dead) return true;
//            t = t.parent;
//        }
//        return false;
//    }
...
//        if (!CameraFrustumTargeter.isEnabled || __instance.maxHorAim == 0f)
//        {
...
//            if (!c.isTrigger || c.gameObject.layer == 22)
//                if (c.TryGetComponent<Coin>(out _) || c.TryGetComponent<Grenade>(out _))
//                {
...
//            bool isThisKojima = col.GetComponent<MassAutoAimHelper>() != null;
//            if (isThisKojima) continue;

//            int layer = col.gameObject.layer;
//            bool isCoin = layer == 10 && col.TryGetComponent<Coin>(out _);
//            bool isGrenade = layer == 14 && col.TryGetComponent<Grenade>(out _);

//            if (col.isTrigger && layer != 22) continue;
//            if (layer == 22 && (!Plugin.hooks.Value || !col.TryGetComponent<HookPoint>(out var hp) || !hp.active)) continue;
//            if (layer == 10 && !isCoin) continue;
//            if (layer == 14 && !isGrenade) continue;
...
//            float score = Vector2.SqrMagnitude(new Vector2(vp.x - 0.5f, vp.y - 0.5f));

//            if (isCoin) score = -1000f;
//            else if (isGrenade) score = -500f;
...
//        if (cube && !cube.GetComponent<MassAutoAimHelper>()) { cube.gameObject.AddComponent<MassAutoAimHelper>(); cube.gameObject.layer = 10; }
